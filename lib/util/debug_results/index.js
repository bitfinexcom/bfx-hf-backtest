...
const genTradesPerDayTable = require('./gen_trades_per_day_table')
...
const debugResults = (btState = {}, summarize = false, isPrimary) => {
  ...
  const {
    showTradeTable, showAggrTradesTable, showPLChart, showTradesPerDayTable,
  ...
  if (showTradesPerDayTable) {
    debug('%s', colors.green('-- Trades Aggregated By Day --'))
    lml(debug, genTradesPerDayTable(strategy).split('\n'))
    debug('')
  }
