 ...
 * console.
 *
 * @param {bfx-hf-strategy.StrategyState[]} strategies - strategies
 * @param {object} args - arguments
 * @param {string} [args.host='ws://localhost:8899'] - HF data server endpoint
 * @param {number|Date} args.from - backtest period start timestamp
 * @param {number|Date} args.to - backtest period end timestamp
 * @param {string} args.symbol - symbol
 * @param {string} args.tf - time frame
 * @param {boolean} [args.candles=true] - whether to include candle data
 * @param {boolean} [args.trades=true] - whether to include trade data
 * @param {boolean} [args.sync=true] - whether to download missing data
 * @param {number} [args.seedPeriod] - number of seed candles, defaults to
 *   minimum value required for all strategy indicators
 * @returns {Promise} p
 */
