...
const execOffline = async (strategy = {}, args = {}, progressCallback) => {
  ...
  const abortStrategy = async (mode) => {
    if (btState.isClosing) {
      return
    }
    btState.isClosing = true

    if (mode === 'CLOSE_AT_MARKET') {
      btState.strategy = await closeOpenPositions(btState.strategy)
    } else {
      btState.strategy = await closePendingOrders(btState.strategy)
    }
  }
  ...
  for (let i = 0; i < btData.length; i += 1) {
    ...
    if (progressCallback) {
      setImmediate(() => progressCallback(i + 1, btData.length))
    }
